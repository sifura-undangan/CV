...
async function sync() {
    ...
    if (document.getElementById('cv-toggle').checked) {
        ...
        // Di dalam fungsi sync() pada script.js
let headerHtml = `<div style="display:flex; ${isPhotoActive ? 'flex-direction:row;' : 'flex-direction:column;'} align-items:center; gap:25px; margin-bottom:12px; width:100%;">
    ...
    <div style="flex:1; display:flex; flex-direction:column; ${isPhotoActive ? 'text-align:left; align-items:flex-start' : 'text-align:center; align-items:center'}; width:100%;">
        ...
        <div style="font-family: 'Times New Roman', serif !important; font-size:11pt !important; color:#000; margin-bottom:8px;">
            ${infoParts.join(' • ')}
        </div>
            ...
            <div style="display:flex; flex-wrap:wrap; gap:15px; ${isPhotoActive ? '' : 'justify-content:center'}">
                ${document.getElementById('in-wa').value ? `
                    <div class="contact-item">
                        <span style="font-family: 'Times New Roman', serif !important; font-size:11pt !important; color:#000;">${document.getElementById('in-wa').value}</span>
                    </div>` : ''}
                
                ${document.getElementById('in-email').value ? `
                    <div class="contact-item">
                        <span style="font-family: 'Times New Roman', serif !important; font-size:11pt !important; color:#000;">${document.getElementById('in-email').value}</span>
                    </div>` : ''}
            </div>

            ${document.getElementById('in-address').value ? `
                <div class="contact-item">
                    <span style="font-family: 'Times New Roman', serif !important; font-size:11pt !important; color:#000;">${document.getElementById('in-address').value}</span>
                </div>` : ''}
        </div>
    </div>
</div>`;
        ...
        // Ganti baris appendSmart untuk profil profesional
if (document.getElementById('in-about').value) {
    appendSmart(`
        <div class="section-title">Profil Profesional</div>
        <div class="cv-profile-text">${document.getElementById('in-about').value}</div>
    `);
            ...
            if (store[c.k].length > 0) {
                ...
                store[c.k].forEach(i => {
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:baseline;">
                        <div style="font-weight:800; font-size:11pt; font-family: 'Times New Roman', Times, serif !important;">${i.t}</div> 
                        <span style="font-size:11pt; font-weight:700; color:#64748b; font-family: 'Times New Roman', Times, serif !important;">${i.s}</span>
                    </div>
                    <div style="color:#4f46e5; font-weight:700; font-size:11pt; margin-top:-1px; font-family: 'Times New Roman', Times, serif !important;">
                        ${i.sub} ${i.extra ? '| ' + i.extra : ''}
                    </div>
